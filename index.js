...
app.get("/cart/:contest_id", (req, res) => {
  new ContestModel().getOneWithProduct(
    "contests.con_id",
...
app.get("/myCart/:user", (req, res) => {
  new ContestModel().getOrders(
    { user_id: req.params.user, status: "oncart" },
    (err, results) => {
      // console.log("results", results);
      if (err) res.send("ERR : " + err);
      else
